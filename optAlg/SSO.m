...
function [sgBestScore,sgBest,GlobalBestCost]=SSO(Problem, Parameters,initPos)

low = Problem.down;
up = Problem.up;
dim = Problem.dim;
Benchmark_Function_ID = Problem.Benchmark_Function_ID;
CostFunction = @(bees) filterFitness(bees,Problem);

n = Parameters.N;
iteration = Parameters.maxIter;
...
if Boundary_no==1
    current_position = initPos(1:n,:);%rand(n,dim).*(up-low)+low; %initial positions in the problem's boundary
end
...
for i = 1:n
    ...
    %////////////////////////////
                                 ...
                                 %-------------------------------------------%      
       %-- position of Swarms ---
    fitness=CostFunction(current_position(i,:));
